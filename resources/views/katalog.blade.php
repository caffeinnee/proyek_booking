    ...
    <body class="font-sans antialiased bg-gray-50 text-gray-900">
        <div class="min-h-screen flex flex-col">
            ...
            <main class="flex-grow">
                ...
                {{-- HERO SECTION --}}
                <section class="relative h-[300px] md:h-[400px] flex items-center justify-center bg-gray-900">
                    ...
                    <div class="relative z-10 w-full max-w-4xl px-4 text-center">
                        ...
                        <p class="text-gray-200 text-base md:text-lg mb-8 max-w-2xl mx-auto">
                            Temukan lapangan terbaik untuk pertandingan serumu di sini.
                        </p>
                        
                        <form action="{{ route('katalog') }}" method="GET">
                            <div class="relative max-w-xl mx-auto group">
                                <div class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                                    <svg class="w-5 h-5 text-gray-400 group-focus-within:text-red-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                                </div>
                ...
                <section class="py-12 md:py-16">
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="flex flex-col md:flex-row justify-between items-end mb-8 border-b border-gray-200 pb-4 gap-4">
                            ...
                            @if($searchTerm)
                                <a href="{{ route('katalog') }}" class="text-sm text-red-600 hover:text-red-800 font-semibold hover:underline flex items-center">
                        ...
                        @if($lapangans->isEmpty())
                            <div class="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-gray-300 shadow-sm">
                                ...
                                <a href="{{ route('katalog') }}" class="mt-6 px-6 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition shadow-md font-medium">Lihat Semua Lapangan</a>
                            </div>
                        @else
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                                @foreach ($lapangans as $lapangan)
                                    <div class="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden h-full transform hover:-translate-y-1
                                        {{ $lapangan->is_featured ? 'border-2 border-yellow-400 ring-4 ring-yellow-50' : 'border border-gray-100' }}">
                                        ...
                                        <a href="{{ route('booking.create', ['lapangan' => $lapangan->id]) }}" class="relative h-56 overflow-hidden bg-gray-100 block">
                                            ...
                                            <div class="absolute top-4 left-4">
                                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-white/90 text-gray-800 backdrop-blur shadow-sm">
                                                    {{ $lapangan->jenis }}
                                                </span>
                                            </div>
                                            ...
                                            @if($lapangan->is_featured)
                                                <div class="absolute top-4 right-4 z-10">
                                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-yellow-400 text-yellow-900 shadow-lg border border-yellow-200 animate-pulse">
                                                        🔥 POPULER
                                                    </span>
                                                </div>
                                            @endif
                                        ...
                                        <div class="p-5 flex flex-col flex-grow">
                                            ...
                                            <div class="flex items-center justify-between mb-3">
                                                <div class="flex items-center text-xs font-medium text-gray-500 truncate pr-2">
                                                    <svg class="w-3.5 h-3.5 mr-1 text-red-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                                                    {{ Str::limit($lapangan->lokasi, 25) }}
                                                </div>
                                                <div class="flex items-center bg-yellow-50 px-2 py-1 rounded text-xs font-bold text-yellow-700 border border-yellow-100">
                                            ...
                                            {{-- TITLE DENGAN CENTANG MERAH YANG RAPI --}}
                                            <div class="flex items-center mb-1">
                                                <h3 class="text-lg font-bold text-gray-900 leading-tight group-hover:text-red-600 transition-colors truncate mr-1 min-w-0">
                                                    <a href="{{ route('booking.create', ['lapangan' => $lapangan->id]) }}" class="truncate block">
                                                        {{ $lapangan->nama_lapangan }}
                                                    </a>
                                                </h3>
                                                
                                                {{-- Ikon Verified Merah --}}
                                                @if($lapangan->user->is_verified)
                                                    <svg class="w-5 h-5 text-red-600 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor" title="Verified Partner">
                                            ...
                                            <p class="text-sm text-gray-500 mb-4 line-clamp-2">
                                                Nikmati pengalaman bermain olahraga yang seru dengan fasilitas lengkap.
                                            </p>
                                            ...
                                            <div class="pt-4 border-t border-gray-100 flex items-center justify-between mt-auto">
                                                <div>
                                                    <span class="text-xs text-gray-400 block mb-0.5">Mulai dari</span>
                                                ...
                                                <a href="{{ route('booking.create', ['lapangan' => $lapangan->id]) }}" 
                                                   class="inline-flex items-center justify-center w-10 h-10 bg-red-50 text-red-600 rounded-full hover:bg-red-600 hover:text-white transition-all shadow-sm group-hover:shadow-md transform group-hover:translate-x-1">
